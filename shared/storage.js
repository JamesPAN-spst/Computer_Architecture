...
function toggleTheme() {
  ...
  localStorage.setItem('architectura-theme', next);
...
(function () {
  try {
    var saved = localStorage.getItem('architectura-theme');
...
function _initNeonCanvas() {
  ...
  var fs = [
    ...
    // 7 palette presets tuned to computer architecture domain colors
    // 0=default iridescent, 1=emerald/electronics, 2=amber/logic,
    // 3=violet/memory, 4=rose/cpu, 5=cyan/software, 6=blue/bus, 7=lavender/gpu
    'vec3 pal(float t){',
    '  float speed=T*0.035;',
    ...
    // emerald (electronics)
    '  vec3 a1=vec3(.15,.52,.42),b1=vec3(.35,.45,.38),c1=vec3(1,1,1),d1=vec3(.30,.55,.45);',
    // amber (logic)
    '  vec3 a2=vec3(.6,.45,.2),b2=vec3(.45,.35,.3),c2=vec3(1,1,1),d2=vec3(.10,.20,.55);',
    // violet (memory)
    '  vec3 a3=vec3(.55,.38,.6),b3=vec3(.45,.35,.45),c3=vec3(1,1,1),d3=vec3(.76,.70,.90);',
    // rose/crimson (cpu)
...
/* ── Progress tracking ── */
var ArchStorage = {
  _key: function (moduleId) { return 'architectura-' + moduleId; },
...
/* ═══════════════════════════════════════════
   ...
   key: architectura-wrong-answers
   ═════════════════════════════════════════ */
var WrongBook = {
  _key: 'architectura-wrong-answers',
